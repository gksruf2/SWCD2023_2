...
def print_network(net):
    ...
    for param, (name, p) in zip(net.parameters(), net.named_parameters()):
        num_params += param.numel()
        #
        print(name)
        print('number of parameters from this net: %.3f M' % (param.numel()/1e6))
    print(net)
...
class ResBlock_M(nn.Module):
    def __init__(self, input_nc, output_nc, hidden_nc=None, norm_layer=nn.BatchNorm2d, nonlinearity= nn.LeakyReLU(),
        ...
        kwargs_s2 = {'kernel_size': 3, 'stride': 2, 'padding': 1}
        kwargs_short = {'kernel_size': 1, 'stride': 1, 'padding': 0}

        self.conv1 = coord_conv(input_nc, hidden_nc, use_spect, use_coord, **kwargs)
        self.conv2 = coord_conv(hidden_nc, output_nc, use_spect, use_coord, **kwargs_s2)
        ...
        self.shortcut = nn.Sequential(self.maxpool, self.bypass,)
...
class MBConv1Block(nn.Module):
    def __init__(self, input_nc, output_nc, hidden_nc=None, norm_layer=nn.BatchNorm2d, nonlinearity= nn.LeakyReLU(),
        ...
        kwargs_110 = {'kernel_size': 1, 'stride': 1, 'padding': 0}
        kwargs_111 = {'kernel_size': 1, 'stride': 1, 'padding': 0}
        kwargs_311 = {'kernel_size': 3, 'stride': 1, 'padding': 1}
        kwargs_short = {'kernel_size': 1, 'stride': 1, 'padding': 0}

        self.conv1 = coord_conv(input_nc, hidden_nc, use_spect, use_coord, **kwargs_111)
        self.conv2 = coord_conv(hidden_nc, output_nc, use_spect, use_coord, **kwargs_311)
        self.conv3 = coord_conv(hidden_nc, output_nc, use_spect, use_coord, **kwargs_111)
        self.bypass = coord_conv(input_nc, output_nc, use_spect, use_coord, **kwargs_short)

        self.expand_conv1 = nn.Sequential(self.conv1, norm_layer(input_nc), nonlinearity)
        self.depthwise_conv1 = nn.Sequential(self.conv2, norm_layer(hidden_nc), nonlinearity)
        
        self.has_se = True
        if self.has_se:
            se_ratio = 0.25
            num_squeezed_channels = max(1,int(input_nc * se_ratio))  # input channel * 0.25 ex) block2 => 16 * 0.25 = 4
            self.conv_se_reduce = coord_conv(hidden_nc, num_squeezed_channels, use_spect, use_coord, **kwargs_110)
            self.conv_se_expand = coord_conv(num_squeezed_channels, hidden_nc, use_spect, use_coord, **kwargs_110)

            self.squeeze_conv1 = nn.Sequential(nn.AdaptiveAvgPool2d(1), self.conv_se_reduce, nonlinearity, 
                                               self.conv_se_expand,)

        self.output_conv1 = nn.Sequential(self.conv3, norm_layer(hidden_nc))
    ...
    def forward(self, x):
        x_ = self.expand_conv1(x)
        x_ = self.depthwise_conv1(x_)
        if self.has_se:
            x_squeeze = self.squeeze_conv1(x_)
            x_ = torch.sigmoid(x_squeeze) * x_ 
        if self.has_dropout:
            x_ = self.dropout(x_)
        x_ = self.output_conv1(x_)

        out = x_ + self.shortcut(x)

        return out

class DN_ResBlock(nn.Module):
    def __init__(self, input_nc, output_nc, hidden_nc=None, norm_layer=nn.BatchNorm2d, nonlinearity= nn.LeakyReLU(),
                 sample_type='none', use_spect=False, use_coord=False):
        super(DN_ResBlock, self).__init__()
