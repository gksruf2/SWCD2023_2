...
class Knowledge_distillation_Loss(nn.Module):
    def __init__(self, alpha=0.1, T=10):
        ...
        self.alpha = alpha  # Rate between student loss and distillation loss
        self.T = T          # Temperature

    def __call__(self, s_pred, t_pred, target):
        #print(type(s_pred), type(t_pred))
        student_loss = F.cross_entropy(s_pred, target)
        #t_pred_scaled  = [pred / self.T for pred in t_pred]
        distllation_loss = nn.KLDivLoss()(F.log_softmax(s_pred/self.T, dim=1),
                                          F.softmax(t_pred/self.T, dim=1)*(self.T*self.T))
        total_loss = self.alpha*student_loss + (1-self.alpha)*distllation_loss
