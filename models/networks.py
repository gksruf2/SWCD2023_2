...
def define_G(opt, image_nc, pose_nc, ngf=64, img_f=1024, encoder_layer=3, norm='batch',
                 activation='ReLU', use_spect=True, use_coord=False, output_nc=3, num_blocks=3, affine=True, nhead=2, num_CABs=2, num_TTBs=2, is_small_model=True):
    print(opt.model)
    if opt.model == 'DPTN':
        ...
        # add is_small_model parameters
        #netG = DPTNGenerator(image_nc, pose_nc, ngf, img_f, encoder_layer, norm, activation, use_spect, use_coord, output_nc, num_blocks, affine, nhead, num_CABs, num_TTBs, is_small_model)
        # change numCAB 2 -> 1, TTB 2 -> 1
        netG = DPTNGenerator(image_nc, pose_nc, ngf, img_f, encoder_layer, norm, activation, use_spect, use_coord, output_nc, num_blocks, affine, nhead, 1, 1, is_small_model)
    else:
...
class DPTNGenerator(nn.Module):
    ...
    def __init__(self, image_nc, pose_nc, ngf=64, img_f=256, layers=3, norm='batch',
        ...
        if is_small_model:
            # ResBlocks -> MBConv1Block
            self.num_blocks = num_blocks
            for i in range(num_blocks):
                block = MBConv1Block(ngf * mult, ngf * mult, norm_layer=norm_layer,
