...
class FID():
    ...
    """
    def __init__(self, minibatch=False, len_images=10000):
        self.dims = 2048
        self.batch_size = 64
        #self.batch_size = 1
        self.cuda = True
    ...
    def compute_statistics_of_path(self, path, verbose):
        ...
        if False:
            ...
            if self.minibatch:
                ...
                m_list = [] # mini batch mean
                s_list = [] # mini batch std
                l_list = [] # mini batch length
                for files_idx in [self.len_images*x for x in range(ceil(len_files / self.len_images))]:
                    end_idx = min(len_files, files_idx+self.len_images)
                    batch_len = end_idx - files_idx
                    batch_files = files[files_idx:end_idx]
                    batch_imgs = np.array([imread(str(fn)).astype(np.float32) for fn in batch_files])
                    ...
                    batch_m, batch_s = self.calculate_activation_statistics(batch_imgs, verbose)
                    m_list.append(batch_m)
                    s_list.append(batch_s)
                    l_list.append(batch_len)                   
                
                m = np.sum([batch_mean*batch_length for batch_mean, batch_length in zip(m_list, l_list)], axis=0)/(len_files)
                s = np.sum([batch_std*(batch_length - 1) for batch_std, batch_length in zip(s_list, l_list)], axis=0)/(len_files-len(l_list))
...
class Reconstruction_Metrics():
    ...
    def __call__(self, inputs, gts):
        ...
        if hasattr(self, 'ssim'):
            ...
            ssim_value = compare_ssim(inputs, gts, data_range=self.data_range, 
                            win_size=self.win_size, channel_axis=-1) 
    ...
    def calculate_from_disk(self, inputs, gts, save_path=None, sort=True, debug=0):
        ...
        if os.path.exists(npz_file):
            ...
            for index in range(len(input_image_list)):
                ...
                #ssim.append(compare_ssim(img_gt, img_pred, data_range=self.data_range, win_size=self.win_size, multichannel=self.multichannel))
                ssim.append(compare_ssim(img_gt, img_pred, data_range=self.data_range, win_size=self.win_size, channel_axis=-1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--minibatch', type=bool)
    parser.add_argument('--len_images', type=int)
    ...
    print('load FID')
    fid = FID(minibatch=args.minibatch, len_images=args.len_images)
    ...
    if args.calculate_mask:
        ...
        mask_lpips_score = lpips.calculate_mask_lpips(distorated_list, gt_list, batch_size=1 ,sort=False)
